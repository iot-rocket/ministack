...
- **CloudFormation — import change sets are validated and never executed** — an import change set ran the create path: executing one emptied the queue being adopted, and one that also renamed a queue the stack owned left the original behind, outside the stack. The change set now lists only the imported resources, as `Import` changes naming the resource, and is refused at creation with AWS's messages when its template adds, changes or removes anything else, an entry names the wrong resource type or identifier key, or an imported resource has no `DeletionPolicy`. An identifier that is blank, names a queue, topic, bucket, table or parameter that does not exist, or names a resource a stack already holds fails the change set with AWS's reason. An import into a stack that does not exist yet creates it in `REVIEW_IN_PROGRESS`, as on AWS. Execution is refused; resource adoption remains unsupported. Contributed by @iot-rocket.
- **CloudFormation — change set changes report their type** — every entry of a `DescribeChangeSet` `Changes` list now carries `Type: Resource`, the one change type, which AWS reports on every change and the emulator left out. Contributed by @iot-rocket.
