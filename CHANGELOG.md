...
- **Lambda — a handler that calls back into MiniStack no longer deadlocks the worker pool** — synchronous executions (Invoke, every execute path, Cognito triggers, custom resources, the AppSync Events authorizer) ran on the shared default thread-pool executor, so a handler's nested call queued behind the invocations waiting on it, deadlocking at exactly the pool size (8-thread pool: 7 re-entrant invokes pass, 8 all time out). Each invocation now gets its own thread. Contributed by @iot-rocket.
